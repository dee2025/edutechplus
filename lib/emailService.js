...
const createTransporter = () => {
  if (!isEmailConfigured()) {
    throw new Error('Email service not configured. Please add SMTP credentials to .env.local');
  }
...
export async function sendNewsletterEmail({ subject, htmlContent, textContent, subscribers }) {
  if (!isEmailConfigured()) {
    console.error('❌ Email not configured. Cannot send newsletter.');
    return { success: false, error: 'Email service not configured. Please add SMTP credentials to .env.local' };
  }
...
export async function sendUnsubscribeEmail(email) {
  if (!isEmailConfigured()) {
    console.warn('⚠️  Email not configured. Skipping unsubscribe confirmation to:', email);
    return { success: false, error: 'Email service not configured', skipped: true };
  }
