...
// NOTE: Raw HTML rendering has been removed. All article content is treated as
// editor content and rendered through the TipTap read-only renderer when the
// article is authored with TipTap/HTML. Otherwise we fall back to Markdown.
function htmlToPlainText(s) {
  if (typeof s !== "string" || s.length === 0) return "";
  // Replace common block tags with newlines
  s = s.replace(/<br\s*\/?>/gi, "\n");
  s = s.replace(/<\/(p|div|h[1-6]|li|ul|ol|blockquote|pre)>/gi, "\n");
...
export default function ArticleContent({ article }) {
  ...
  const content = htmlToPlainText(contentRaw);

  // Consider content TipTap if explicitly marked or if content contains HTML tags
  const isTipTap =
    (article && article.content_format === "tiptap") ||
    /<[^>]+>/i.test(contentRaw);
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      >
        {isTipTap ? (
          <TipTapRenderer content={contentRaw} fallback={content} />
        ) : (
          <ReactMarkdown
